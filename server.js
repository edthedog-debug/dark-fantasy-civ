...
// Rate limiter - 20 seconds between calls to stay within Groq free tier
const rateLimiter = {
    lastCallTime: 0,
    minInterval: 20000,
...
/**
 * AI Events - With Groq AI connection (NO fallbacks)
 */
async function generateAIEvents() {
    ...
    const prompt = `Generate a dark fantasy civilization event. Day ${worldState.day}, Population ${worldState.population}, Treasury ${treasury}. Return JSON with event, goldImpact (negative percentage of treasury), happinessImpact, techImpact, visualEffect, duration.`;
    ...
    if (aiResult) {
        try {
            ...
            console.log("⚠️ AI response parse failed:", e.message);
        }
    } else {
        console.log("⚠️ Groq unavailable - skipping event this cycle");
        addLog("[AI EVENT] Groq unavailable - event skipped");
    }
...
function runSimulationTick() {
    ...
    // AI Event every 150 days (10 minutes) - within rate limit
    if (worldState.day % 150 === 0) {
        generateAIEvents();
    }

    // Code improvement every 250 days (17 minutes) - within rate limit
...
async function autoImproveGameCode() {
    ...
    try {
        ...
        if (aiResponse && aiResponse.length > 50) {
            ...
            addLog("[AI] Groq unavailable - improvement skipped");
        }
...
SERVER.listen(PORT, () => {
    ...
    console.log("⏱️ Events: every 150 days (10 min) | Code: every 250 days (17 min)");
    console.log("🔧 Rate limiter: 20s between AI calls");
    ...
    queryAI("Say OK", "CONNECTION TEST").then(response => {
        if (response) {
            ...
            addLog("[SYSTEM] AI unavailable - will retry next cycle.");
        }
        broadcastState();
    });
});
